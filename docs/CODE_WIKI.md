...
> _Based on Google ADK & Reliability Engineering Standards_

Welcome to the project's digital brain. This wiki breaks down the architecture, design patterns, and internal logic of the Chaos Engine. It is designed to provide a deep understanding of _why_ the system is robust, modular, and efficient.
...
graph TD
    ...
    Agent ==>|⚠️ Exception Caught| Memory[PlaybookStorage] # Thick arrow represents the Catch
...
> - **⚠️ Transparency Note:** The audit detected that while CSV writing is streamed, an `all_results_buffer` exists that accumulates metrics for the final report. _Optimization Opportunity:_ Implement streaming aggregation to eliminate this buffer in simulations \>1M events.
...
> **💎 Software Quality:**
>
> - **🧱 Pillar III (Modularity):** Decouples the _execution context_ (CLI) from the _business logic_ (`ParametricABTestRunner`). This allows the simulation core to be imported as a library without side effects.
...
> **💎 Software Quality:**
>
> - **🧱 Pillar III (Dependency Injection):** This class is the _Composition Root_. It manually constructs the object graph (`ChaosProxy` -> `CircuitBreaker` -> `ToolExecutor` -> `Agent`), ensuring that dependencies are injected explicitly rather than hardcoded.
...
> **💎 Software Quality:**
>
> - **🛡️ Pillar IV (Reliability Testing):** Implements **Semantic Evaluation**. Unlike simple pass/fail tests, this runner uses an LLM-as-a-Judge approach (via ADK) to verify the _quality_ of the agent's decision-making process.
> - **🚀 Innovation (Phase 6):** This class represents the shift from _deterministic_ simulated testing to _probabilistic_ evaluation, essential for validating the reasoning capabilities of Gemini 2.0.
...
> **💎 Engineering Standards:**
>
> - **🧠 Pillar I (Cognitive):** Decouples **recovery logic** (JSON data) from **execution logic** (Python code). This allows updating resilience strategies without redeploying the agent code.
...
4.  _Automatically_, `ChaosProxy` will be able to inject failures into it without additional changes.
...
3.  **Result:** The `ChaosProxy` will now inject failures into _real_ network calls, validating resilience under actual network conditions.
