"""
src/chaos_playbook_engine/agents/test_agent_adk.py

TestAgent como verdadero ADK Agent (LlmAgent)

ARQUITECTURA:

    TestAgent (LlmAgent ADK con Gemini)
        ↓
        Lee playbook
        Usa tools: getPetById, check_playbook, decide_retry, apply_backoff
        ↓
        Cuando test falla:
        - Consulta playbook para error_code
        - LLM razona: ¿reintentar?
        - Ejecuta decisión (backoff + reintento)
        ↓
        Retorna: resultado con historial de intentos y decisiones

KEY POINT:
    No es una clase artesana.
    Es un LlmAgent ADK que razona con Gemini.
    Visible en ADK Web UI con tracing.
    SessionService mantiene historial de conversación.
"""
...
class TestAgentADK(LlmAgent):
    """
    ADK Agent for testing with chaos engineering.
    ...
    def __init__(
        ...
        """
        Initialize TestAgent as an ADK LlmAgent.
        
        Args:
            chaos_agent_func: Callable that injects chaos
...
"""
    ...
    async def run_test(self, test_spec: Dict[str, Any], attempt: int = 1) -> Dict[str, Any]:
        ...
        This method demonstrates how TestAgent uses tools and LLM reasoning.
    ...
    async def _tool_call_operation(
        ...
        logger.info(f"   → Tool: calling {operation_id}({params})")
        self.tool_calls += 1
        
        # Call ChaosAgent through provided function
        result = self.chaos_agent_func(
            operation_id=operation_id,
            params=params,
    ...
    async def _tool_check_playbook(
        ...
        - action: 'retry', 'fail', 'circuit_break'
        - max_retries: Maximum retry attempts
        - backoff_seconds: Backoff base value
        """
        
        logger.info(f"   → Tool: checking playbook for error {error_code}")
        
        # Search playbook
        for procedure in self.playbook.get('procedures', []):
            if procedure['error_code'] == str(error_code):
                result = {
                    'found': True,
    ...
    async def _tool_decide_retry(
        ...
        # Simple logic: LLM would normally reason here
        # For now, simple heuristic for demo
    ...
    async def _tool_apply_backoff(
        ...
        logger.info(f"   → Tool: applying backoff ({seconds}s)")
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        passed = sum(1 for r in self.test_results if r['ok'])
