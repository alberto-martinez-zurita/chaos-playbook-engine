"""
src/chaos_playbook_engine/agents/test_agent_adk_v2.py - FIXED HttpRetryOptions

TestAgent como verdadero ADK Agent (LlmAgent) - PYDANTIC COMPATIBLE + FIXED RetryOptions

PROBLEMA ANTERIOR:
    ValidationError: 3 validation errors for HttpRetryOptions
    - expbase
    - initialdelay
    - httpstatuscodes
    
CAUSA:
    Los parámetros no eran correctos para ADK HttpRetryOptions
    
SOLUCIÓN:
    Usar parámetros válidos o simplemente no pasar retry options
...
class TestAgentADK(LlmAgent):
    """
    ADK Agent for testing with chaos engineering - PYDANTIC COMPATIBLE VERSION (FIXED).
    ...
    FIXED: HttpRetryOptions parámetros correctos
    """
    ...
    def __init__(
        ...
        **kwargs
        ...
        """
        Initialize TestAgent as an ADK LlmAgent - PYDANTIC COMPATIBLE (FIXED).
...
"""
        ...
        # Initialize LlmAgent WITHOUT retry options (to avoid Pydantic validation errors)
        # ADK will use default retry behavior
        try:
            super().__init__(
                name="TestAgentADK",
                model=Gemini(
            ...
            logger.error(f"Failed to initialize LlmAgent: {e}")
            raise
